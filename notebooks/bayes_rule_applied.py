...
# Sleep data
plt.scatter(
    sleep_data['time_offset'], sleep_data['indicator'],
...
figsize(16, 6)
plt.plot(
    time_est, sleep_est, color = 'navy',
    lw=4, label='Model',
)
plt.scatter(
    time, sleep_obs, edgecolor = 'slateblue',
...
print(
    '9:30  PM probability of being asleep: {:.2f}%.'.
    format(100 * calculate_prior(-30, alpha, beta)),
)
print(
    '10:00 PM probability of being asleep: {:.2f}%.'.
    format(100 * calculate_prior(0, alpha, beta)),
)
print(
    '10:30 PM probability of being asleep: {:.2f}%.'.
    format(100 * calculate_prior(30, alpha, beta)),
)
...
figsize(18, 8)
plt.plot(
    time_est, sleep_est, color = 'black',
    lw=3, linestyle = '--', label='Prior Probability',
)
plt.plot(
    time_est, light_on_prob, color = 'darkred',
    lw = 5, label = 'Light On Probability',
)
plt.plot(
    time_est, light_off_prob, color = 'darkgreen',
...
def add_update_probability(time_offset, light, phone):
    ...
    if type(time_offset) == int:
        ...
        print(
            'Time is {} PM \tLight is {} \tPhone IS {}.'.format(
...
plt.plot(
    time_est, light_phone, color = 'orange', lw = 4,
    label = 'Light ON Phone Charging',
)
plt.plot(
    time_est, nolight_phone, color = 'darkmagenta', lw = 4,
    label = 'Light OFF Phone Charging',
)
plt.plot(
    time_est, light_nophone, color = 'brown', lw= 4,
    label = 'Light ON Phone NOT Charging',
)
plt.plot(
    time_est, nolight_nophone, color = 'darkblue', lw = 4,
